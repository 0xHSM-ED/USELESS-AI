// ---------- the "brain" (ported straight from main.cpp, no server needed) ----------
const CHOICES = [
...
const GREETING_TRIGGERS = ["hi", "hello", "hey", "yoo", "sup"];
const SPECIAL_RESPONSES = {
  ...
  "linux": [
    ...
    "you probably broke something with sudo.",
    "it works on my machine.",
    "Arch users have entered the chat.",
    "dependency hell awaits."
  ],
  ...
  "ai": [
    ...
    "I have approximately 0.2 intelligence.",
...
function evalAnswer(message) {
  ...
  for (const word in SPECIAL_RESPONSES) {
    if (lower.includes(word)) {
      return pickChoice(SPECIAL_RESPONSES[word]);
    }
  }

  const isAngry = ANGER_TRIGGERS.some((word) => lower.includes(word));
  const isGreeting = GREETING_TRIGGERS.some((word) => lower.includes(word));

  if (isAngry) {
    return pickChoice(ANGER_RESPONSES);
  }

  return isGreeting ? pickChoice(GREETINGS) : pickChoice(CHOICES);
}
...
const ICON_MOON = "\uf186"; // nf-fa-moon
const ICON_SUN = "\uf185";  // nf-fa-sun-o
...
async function sendMessage() {
  ...
  // small fake "thinking" delay so it doesn't feel instant/robotic
  const thinkTime = 250 + Math.random() * 350;
  setTimeout(() => {
    typewrite(aiBody, evalAnswer(message));
  }, thinkTime);
}
